...
export default class MeasurementApi {
  ...
  static syncMeasurementAndToolData(measurement) {
    ...
    // State of the image taken from cornerstone
    const toolState = cornerstoneTools.globalImageIdSpecificToolStateManager.saveToolState();
    localStorage.setItem('toolState', JSON.stringify(toolState));
    // Variable to be passed in order the annotations to be saved in the database
    const stateForDB = localStorage.getItem('toolState');
  ...
  //This function has been modified to retrieve the annotations saved on the database based on the currently logged user
    ...
    return new Promise((resolve, reject) => {
      retrievalFn(server).then(measurementData => {
        if (measurementData) {
          ...
          console.log("Retrieving annotation");
          ...
          //AJAX call to get the saved annotations from the database
          var return_first = function () {
            var tmp = null;
            $.ajax({
              type: "GET",
              async: false,
              url: 'http://51.195.202.88/lyankova/dicom-image-viewer/db/database.php?function=retrieveAnnotation',
              data: ({name: username}),
              success: function(response){
                //alert("Annotations retrieved!");
                tmp = response;
              },
          ...
          let toolState = JSON.parse(return_first);
          if(!toolState)
          {
            return;
          }
          console.log(toolState);
          let existingState = cornerstoneTools.globalImageIdSpecificToolStateManager.saveToolState(toolState);
          console.log(existingState);

          //for each loop that goes through the annotation and saves specific data
          Object.keys(toolState).forEach(instanceId => {
            const instances = toolState[instanceId];
            Object.keys(instances).forEach(measurementTypeId => {
              const measurements = instances[measurementTypeId]["data"];
              Object.keys(measurements).forEach(DataId => {
                const measurement =  measurements[DataId]
                this.addMeasurement(measurementTypeId, measurement);
              });
            });
          });
